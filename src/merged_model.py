...
class CompletedModel(object):
    ...
    def recognize(self, image):
        ...
        for i in range(len(result)):
            print(result[i].encode("utf-8").decode("utf-8"))
        print("result", result)
        field_dict['id'] = result[0]
        field_dict['ten'] = (' '.join(result[1:len(self.name_boxes) + 1])).encode("utf-8").decode()
        field_dict['Ngay sinh'] = (result[len(self.name_boxes) + 1]).encode("utf-8").decode()
        field_dict['Que quan'] = (' '.join(result[len(self.name_boxes) + 2: -len(self.home_boxes)])).encode("utf-8").decode()
        print(result[len(self.name_boxes) + 2: -len(self.home_boxes)])
        field_dict['Dia chi'] = (' '.join(result[-len(self.home_boxes):])).encode("utf-8").decode()
